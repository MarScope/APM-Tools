...
| `ATEL(device_bot): device_id= action=... reason=...` (empty id) | Profile attached but **Device ID Mode is *None*** (the profile default), or the client hasn't completed the JS challenge yet (first request from a new browser never has an ID). An empty id together with `reason=Passed Redirect Challenge` is conclusive: browser verification is working but no device ID is being generated — set **Device ID Mode** to *Generate Before Access* or *Generate After Access* in the Bot Defense profile (Security › Bot Defense › Bot Defense Profiles › *profile*, Advanced view) and note that IDs appear on the requests *after* the device-ID JavaScript has run. |
